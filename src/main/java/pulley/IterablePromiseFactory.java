...
public class IterablePromiseFactory<T> implements Factory<Promise<Optional<Cons<T>>>> {
    ...
    private Factory<Promise<Optional<Cons<T>>>> createPromiseFactory(final Iterator<T> it) {
        return new IteratorPromiseFactory<T>(it);
    }

    private static class IteratorPromiseFactory<T> implements Factory<Promise<Optional<Cons<T>>>> {
        ...
        public Promise<Optional<Cons<T>>> create() {
            ...
            ref.set(new FunctionPromise<Optional<Cons<T>>>(new F0<Optional<Cons<T>>>() {
                @Override
