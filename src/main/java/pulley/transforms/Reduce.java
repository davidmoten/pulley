...
public class Reduce {
	...
	private static class ReduceTransformer<T, R> implements Transformer<T, R> {
		...
		public ReduceTransformer(R initial,
				F2<? super R, ? super T, ? extends R> reducer) {
		...
		@Override
		public Promise<Optional<Cons<R>>> transform(
				final Promise<Optional<Cons<T>>> promise) {
			return new AbstractStreamPromise<T, R>(promise) {
				...
				public Optional<Cons<R>> get() {
					final AtomicReference<R> ref = new AtomicReference<R>(
							initial);
					A1<T> action = t -> ref.set(reducer.call(ref.get(), t));
